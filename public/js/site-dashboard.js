...
var GoogleAnaylytics =
/*#__PURE__*/
function () {
  function GoogleAnaylytics() {
    _classCallCheck(this, GoogleAnaylytics);
  }

  _createClass(GoogleAnaylytics, null, [{
    key: "init",
    value: function init() {
      window.chartColors = {
        red: 'rgb(255, 99, 132)',
      ...
      gapi.analytics.ready(function () {
        ...
        function renderWeekOverWeekChart(ids) {
          ...
          var now = __webpack_provided_window_dot_moment(); // .subtract(3, 'day');

          var thisWeek = query({
            'ids': ids,
            'dimensions': 'ga:date,ga:nthDay',
            'metrics': 'ga:sessions',
            'start-date': __webpack_provided_window_dot_moment(now).subtract(1, 'day').day(0).format('YYYY-MM-DD'),
            'end-date': __webpack_provided_window_dot_moment(now).format('YYYY-MM-DD')
          });
          var lastWeek = query({
            'ids': ids,
            'dimensions': 'ga:date,ga:nthDay',
            'metrics': 'ga:sessions',
            'start-date': __webpack_provided_window_dot_moment(now).subtract(1, 'day').day(0).subtract(1, 'week').format('YYYY-MM-DD'),
            'end-date': __webpack_provided_window_dot_moment(now).subtract(1, 'day').day(6).subtract(1, 'week').format('YYYY-MM-DD')
          });
          Promise.all([thisWeek, lastWeek]).then(function (results) {
            ...
            labels = labels.map(function (label) {
              return __webpack_provided_window_dot_moment(label, 'YYYYMMDD').format('ddd');
            });
            var data = {
              labels: labels,
              datasets: [{
        ...
        function renderYearOverYearChart(ids) {
          ...
          var thisYear = query({
            ...
            'metrics': 'ga:users',
            'start-date': __webpack_provided_window_dot_moment(now).date(1).month(0).format('YYYY-MM-DD'),
            'end-date': __webpack_provided_window_dot_moment(now).format('YYYY-MM-DD')
          });
          var lastYear = query({
            'ids': ids,
            'dimensions': 'ga:month,ga:nthMonth',
            'metrics': 'ga:users',
            'start-date': __webpack_provided_window_dot_moment(now).subtract(1, 'year').date(1).month(0).format('YYYY-MM-DD'),
            'end-date': __webpack_provided_window_dot_moment(now).date(1).month(0).subtract(1, 'day').format('YYYY-MM-DD')
          });
          Promise.all([thisYear, lastYear]).then(function (results) {
            ...
            var labels = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']; // Ensure the data arrays are at least as long as the labels array.
        ...
        function renderTopBrowsersChart(ids) {
          var now = __webpack_provided_window_dot_moment();
          query({
            'ids': ids,
            ...
            data = {
              datasets: [{
                data: chartData,
                backgroundColor: [window.chartColors.red, window.chartColors.orange, window.chartColors.yellow, window.chartColors.green, window.chartColors.blue, window.chartColors.purple, window.chartColors.grey]
              }],
              labels: labels
            };
        ...
        function renderTopCountriesChart(ids) {
          ...
          query({
            ...
            var chartData = [];
            var labels = [];
            var colors = ['rgb(255, 99, 132)', 'rgb(255, 159, 64)', 'rgb(255, 205, 86)', 'rgb(75, 192, 192)', 'rgb(54, 162, 235)', 'rgb(153, 102, 255)', 'rgb(231,233,237)'];
            var bgColors = [];
            var colorCounter = 0;
            response.rows.forEach(function (row, i) {
...
module.exports = __webpack_require__(/*! C:\Apache24\httpd-2.4.41-win64-VS16\Apache24\htdocs\school-management-system-master\resources\assets\backend\js\site-dashboard.js */"./resources/assets/backend/js/site-dashboard.js");
